...
defmodule Day12Part1 do
  ...
  defp get_a_position(grid) do
    ...
    maps = grid |> Enum.map(fn x -> Enum.find_index(x, a) end)
  ...
  defp solve(grid, {_, {current, steps}}, ending, queue, set) do
    ...
    queue = tos |> Enum.reduce(queue, fn {x, y}, q ->
      current = grid |> Enum.at(elem(current, 0)) |> Enum.at(elem(current, 1))
      to = grid |> Enum.at(x) |> Enum.at(y)
      PriorityQueue.put(q, abs(ex - x) + abs(ex - y) + steps + 1 + (current - to), {{x, y}, steps + 1})
     end)
    {next, queue} = PriorityQueue.pop(queue)
